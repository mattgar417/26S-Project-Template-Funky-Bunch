...
from backend.owner.owner_routes import venue_routes
...
def create_app():
    ...
    app.config["MYSQL_DATABASE_PASSWORD"] = os.getenv("MYSQL_ROOT_PASSWORD", "root_password").strip()
    app.config["MYSQL_DATABASE_HOST"] = os.getenv("DB_HOST", "db").strip()
    app.config["MYSQL_DATABASE_PORT"] = int(os.getenv("DB_PORT", "3306").strip())
    app.config["MYSQL_DATABASE_DB"] = os.getenv("DB_NAME", "CommunEvent_db").strip()
    ...
    app.register_blueprint(venue_routes, url_prefix="/owner")
